...
async function load(){try{showApp(await api('/v1/workspace/overview'));await Promise.all([loadWorkflow(),loadDocuments(),loadFinancialOverview(),loadCountryReadiness(),loadTeam(),loadAudit()]);}catch(error){session=null;sessionStorage.removeItem('opsnestWorkspaceSession');$('loginView').classList.remove('hidden');$('appView').classList.add('hidden');$('logout').classList.add('hidden');$('loginError').textContent=error.message;}}
