...
import json
from .workspace_i18n import BG_TRANSLATIONS
...
<option value="en">English</option><option value="bg">Български</option>
</select>
...
<h1 id="companyName" data-i18n-ignore>Your OpsNest workspace</h1>
...
const LANGUAGE_KEY='opsnestWorkspaceLanguage';let language=localStorage.getItem(LANGUAGE_KEY)||'sr';if(!['sr','en','bg'].includes(language))language='sr';
const BG_TRANSLATIONS=__OPSNEST_BG_TRANSLATIONS__;
const SR_TRANSLATIONS=Object.freeze({
 'Founder — no package limits':'Founder — bez paketnih ograničenja','Unlimited':'Neograničeno',
...
const SR_DOCUMENT_ACCESS_TRANSLATIONS=Object.freeze({'Your role does not have access to the document archive.':'Vaša uloga nema pristup arhivi dokumenata.','Too many sign-in attempts. Wait 15 minutes or reset the password.':'Previše pokušaja prijave. Sačekajte 15 minuta ili obnovite lozinku.','Too many invitation code attempts. Request a new invitation.':'Previše pokušaja koda pozivnice. Zatražite novu pozivnicu.'});
...
const SR_ALL={...SR_TRANSLATIONS,...SR_DASHBOARD_TRANSLATIONS,...SR_CONTROL_TRANSLATIONS,...SR_COUNTRY_PACK_TRANSLATIONS,...SR_AUTOMATION_TRANSLATIONS,...SR_DOCUMENT_ACCESS_TRANSLATIONS,...SR_DOCUMENT_STORAGE_STATUS_TRANSLATIONS,days:'dana','Download audit evidence':'Preuzmi audit dokaz','Open section':'Otvori','International core':'Međunarodna osnova','Audit integrity verified':'Integritet audit traga je potvrđen',events:'događaja',active:'aktivan',invited:'pozvan',revoked:'pristup ukinut',foundation:'osnova','Open overdue receivables':'Dospela potraživanja','VAT payable':'PDV za plaćanje'};
const REVERSE_TEXT=Object.fromEntries([...Object.entries(SR_ALL),...Object.entries(BG_TRANSLATIONS)].map(([key,value])=>[value.trim(),key.trim()]));
function baseText(value){const source=String(value??'');return REVERSE_TEXT[source.trim()]||source.trim();}
function tr(value){const source=String(value??''),base=baseText(source),catalog=language==='bg'?BG_TRANSLATIONS:language==='sr'?SR_ALL:{},counted=base.match(/^([0-9]+) (.+)$/);const translated=catalog[base]||(counted&&catalog[counted[2]]?counted[1]+' '+catalog[counted[2]]:base);return source.replace(source.trim(),translated);}
function localizePortal(){document.documentElement.lang=language;document.title=language==='bg'?'OpsNest работно пространство':language==='sr'?'OpsNest radni prostor':'OpsNest Workspace';document.querySelectorAll('body *').forEach(element=>{if(element.closest('[data-i18n-ignore]')||['SCRIPT','STYLE','TEXTAREA'].includes(element.tagName))return;for(const node of element.childNodes){if(node.nodeType!==3||!node.nodeValue.trim())continue;if(node.__opsnestRendered!==node.nodeValue)node.__opsnestBase=node.nodeValue;const base=element.getAttribute('data-i18n-en')||node.__opsnestBase;const translated=language==='sr'&&element.hasAttribute('data-i18n-sr')?element.getAttribute('data-i18n-sr'):tr(base);if(node.nodeValue!==translated)node.nodeValue=translated;node.__opsnestRendered=translated;}for(const attr of ['placeholder','aria-label']){if(!element.hasAttribute(attr))continue;const key='opsnestBase'+attr;if(!element[key])element[key]=element.getAttribute(attr)||'';const translated=tr(element[key]);if(element.getAttribute(attr)!==translated)element.setAttribute(attr,translated);}});}

// Translate controlled labels only; names, filenames and free-text notes are verbatim.
const label=value=>`<span>${esc(value)}</span>`;
const verbatim=value=>`<span data-i18n-ignore>${esc(value)}</span>`;
const serverLabel=(value,sr)=>`<span data-i18n-en="${esc(value)}" data-i18n-sr="${esc(sr||value)}">${esc(value)}</span>`;
...
function renderWorkflow(data){workflowItems=Object.fromEntries(data.items.map(item=>[item.id,item]));workflowCanManage=Boolean(data.can_manage);$('queueForm').classList.toggle('hidden',!workflowCanManage);$('queueAssignee').innerHTML='<option value="">Unassigned</option>'+data.members.map(member=>`<option data-i18n-ignore value="${esc(member.id)}">${esc(member.display_name||member.email)}</option>`).join('');$('documentWorkflow').innerHTML='<option value="">No work item</option>'+data.items.map(item=>`<option data-i18n-ignore value="${esc(item.id)}">${esc(item.title)}</option>`).join('');if(!data.items.length){$('workflowList').innerHTML='<p class="muted">No work items yet. Start with the next payment, document or VAT check.</p>';localizePortal();return;}$('workflowList').innerHTML=data.items.map(item=>{const status=workflowCanManage?`<select class="workflow-status" data-id="${esc(item.id)}">${workflowOptions(item.status)}</select>`:`<span class="status">${esc(workflowLabel[item.status]||item.status)}</span>`;const due=item.due_date?` · ${label('due')} ${esc(item.due_date)}`:'';return `<article class="queue-item"><div><b data-i18n-ignore>${esc(item.title)}</b><p>${label(workflowLabel[item.workflow_type]||item.workflow_type)} · ${label(workflowLabel[item.priority]||item.priority)}${due} · ${verbatim(item.assigned_member_name)}</p></div><div class="queue-actions">${status}<button class="quiet small workflow-comment" data-id="${esc(item.id)}" type="button">${label("Comment")} (${item.comment_count})</button><button class="quiet small workflow-history" data-id="${esc(item.id)}" type="button">History</button></div></article>`;}).join('');localizePortal();document.querySelectorAll('.workflow-status').forEach(control=>control.addEventListener('change',async event=>{const item=workflowItems[event.target.dataset.id],status=event.target.value;let comment='';if(status==='returned'){comment=window.prompt(tr('Explain what must be corrected. Do not enter passwords, invoice files or payment credentials.'))||'';if(comment.trim().length<3){$('queueStatus').textContent=tr('A correction comment is required.');await loadWorkflow();return;}}try{await api('/v1/workflow-items/'+item.id,{method:'PATCH',body:JSON.stringify({status,priority:item.priority,due_date:item.due_date,assigned_member_id:item.assigned_member_id,comment:comment.trim()})});await loadWorkflow();}catch(error){$('queueStatus').textContent=error.message;await loadWorkflow();}}));document.querySelectorAll('.workflow-comment').forEach(button=>button.addEventListener('click',()=>addWorkflowComment(button.dataset.id)));document.querySelectorAll('.workflow-history').forEach(button=>button.addEventListener('click',()=>showWorkflowHistory(button.dataset.id)));}
...
async function revokeTeamMember(id){if(!window.confirm(tr('Revoke this person’s access immediately? Their active sessions will be signed out.')))return;try{await api('/v1/team/members/'+id+'/revoke',{method:'POST'});await loadTeam();}catch(error){$('teamStatus').textContent=error.message;}}
function renderSessions(data){$('sessionsList').innerHTML=data.sessions.map(item=>`<article class="queue-item"><div><b>${verbatim(item.device_name)}${item.current?' · '+esc(tr('Current device')):''}</b><p>${verbatim(item.member_name)} · ${esc(item.last_seen_at.replace('T',' '))}</p></div><div class="queue-actions"><button class="quiet small revoke-session" data-id="${esc(item.id)}" type="button">${esc(tr('Revoke device'))}</button></div></article>`).join('')||'<p class="muted">—</p>';document.querySelectorAll('.revoke-session').forEach(button=>button.addEventListener('click',()=>revokeTeamSession(button.dataset.id)));}
...
function renderAudit(data){$('auditList').innerHTML=data.events.slice(0,20).map(event=>`<article class="queue-item"><div><b>${esc(auditLabel(event.action))}</b><p>${esc(event.at.replace('T',' ').slice(0,19))} · ${verbatim(event.actor_name)}</p></div></article>`).join('')||'<p class="muted">No operational events recorded yet.</p>';localizePortal();}
async function loadAudit(){if(!teamCanManage)return;try{const [audit,integrity]=await Promise.all([api('/v1/team/audit'),api('/v1/team/audit/integrity')]);renderAudit(audit);$('auditIntegrity').className='muted';$('auditIntegrity').innerHTML=`${label('Audit integrity verified')} · ${integrity.count} ${label('events')}.`;}catch(error){$('auditIntegrity').className='error';$('auditIntegrity').textContent=error.message;$('auditList').innerHTML=`<p class="error">${esc(error.message)}</p>`;}}
...
let controlBriefData=null,financialOverviewData=null;
function renderControlBrief(data){controlBriefData=data;const labels={attention:'Attention',watch:'Watch',clear:'Clear'},items=Array.isArray(data.items)?data.items:[];$('controlBriefList').innerHTML=items.map(item=>{const title=serverLabel(item.title,item.title_sr),detail=serverLabel(item.detail,item.detail_sr),open=item.target?`<button class="quiet small brief-open" data-target="${esc(item.target)}" type="button">${esc(tr('Open section'))}</button>`:'';return `<article class="queue-item"><div><b>${title}</b><p>${detail}</p></div><div class="queue-actions"><span class="status ${esc(item.severity)}">${esc(tr(labels[item.severity]||'Watch'))}</span>${open}</div></article>`;}).join('')||'<p class="muted">—</p>';$('controlBriefStatus').innerHTML=serverLabel(data.disclaimer||'Operational prompt only — review the relevant controlled workflow before taking action.',data.disclaimer_sr);document.querySelectorAll('.brief-open').forEach(button=>button.addEventListener('click',()=>{const target=$(button.dataset.target);if(target)target.scrollIntoView({behavior:'smooth',block:'start'});}));}
...
function money(value,currency){return new Intl.NumberFormat(({sr:'sr-RS',bg:'bg-BG',en:'en-US'})[language],{style:'currency',currency:currency||'EUR',maximumFractionDigits:2}).format(Number(value||0));}
function renderFinancialControl(data){const summary=data.summary;if(!summary){$('financialActions').innerHTML='<p class="muted">—</p>';$('financialControlStatus').textContent=tr('Synchronize the Desktop financial overview to generate the first control brief.');localizePortal();return;}const currency=data.currency||summary.currency||'EUR',actions=[],positive=value=>Number(value||0)>0;if(positive(summary.overdue_receivables))actions.push(['Overdue collection needs review',`${label('Open overdue receivables')}: ${esc(money(summary.overdue_receivables,currency))}. ${label('Review open receivables in Desktop, confirm collection dates and create one owner task for every material exception.')}`,'Attention','attention']);if(positive(summary.open_payables))actions.push(['Open supplier obligations need a payment plan',`${label('Open payables')}: ${esc(money(summary.open_payables,currency))}. ${label('Review supplier due dates and cash availability before any payment is prepared.')}`,'Watch','watch']);if(Number(summary.forecast_closing||0)<0)actions.push(['Forecast shows a liquidity risk',`${label('Forecast closing')}: ${esc(money(summary.forecast_closing,currency))}. ${label('Compare the forecast with planned commitments before approving new spending.')}`,'Attention','attention']);if(positive(summary.vat_payable))actions.push(['VAT amount needs period control',`${label('VAT payable')}: ${esc(money(summary.vat_payable,currency))}. ${label('Reconcile the period with the accountant before filing or paying any VAT obligation.')}`,'Watch','watch']);if(!actions.length)actions.push(['No aggregate exception detected',label('The synchronized totals do not show an urgent control exception. Continue the normal review cycle.'),'Clear','']);$('financialActions').innerHTML=actions.map(([title,detail,status,style])=>`<article class="queue-item"><div><b>${esc(title)}</b><p>${detail}</p></div><span class="status ${style}">${esc(status)}</span></article>`).join('');$('financialControlStatus').innerHTML=`${label('Updated')} ${esc(String(data.updated_at||'').replace('T',' '))} · ${Number(summary.horizon_days||data.horizon_days||90)} ${label('days')}.`;localizePortal();}
function renderFinancialOverview(data){financialOverviewData=data;const summary=data.summary;if(!summary){$('financialOverview').innerHTML='<article class="card metric"><span>Financial overview</span><b>—</b></article>';$('financialOverviewStatus').textContent=tr(data.message||'No Desktop financial summary has been synchronized yet.');renderFinancialControl(data);localizePortal();return;}const currency=data.currency||summary.currency||'EUR';$('financialOverview').innerHTML=[['Income without VAT',summary.income_net],['Expenses without VAT',summary.expense_net],['Net result',summary.profit_net],['Open receivables',summary.open_receivables],['Open payables',summary.open_payables],['Forecast closing',summary.forecast_closing]].map(([label,value])=>`<article class="card metric"><span>${esc(label)}</span><b>${esc(money(value,currency))}</b></article>`).join('');$('financialOverviewStatus').innerHTML=`${label('Desktop summary is ready.')} ${label('Updated')} ${esc(String(data.updated_at||'').replace('T',' '))} · ${Number(summary.horizon_days||data.horizon_days||90)} ${label('days')}.`;renderFinancialControl(data);localizePortal();}
...
function showApp(data){$('loginView').classList.add('hidden');$('appView').classList.remove('hidden');$('logout').classList.remove('hidden');currentMemberId=data.member.id;teamCanManage=Boolean(data.team.can_manage);$('teamSection').classList.toggle('hidden',!teamCanManage);$('sessionsSection').classList.toggle('hidden',!teamCanManage);$('auditSection').classList.toggle('hidden',!teamCanManage);$('companyName').textContent=data.workspace.company_name||'OpsNest workspace';$('heroCopy').innerHTML=[data.member.role_label,data.workspace.country_label,data.workspace.country_pack_stage].map(label).join(' • ');
 const sync=data.sync.enabled?`${label('Revision')} ${Number(data.sync.revision)||0}`:label('Not connected yet');
 const plan=data.license.access_source==='founder'?'Founder — no package limits':data.license.plan_name||data.license.effective_plan_code||data.license.plan_code;
 const seats=`${Number(data.team.seats_used)||0} / `+(data.team.seat_limit===null?label('Unlimited'):esc(data.team.seat_limit));
 $('metrics').innerHTML=[['Plan',plan],['Team seats',seats],['Cloud sync',sync],['Role',data.member.role_label]].map(([key,value])=>`<article class="card metric"><span>${esc(key)}</span><b>${key==='Cloud sync'||key==='Team seats'?value:esc(value)}</b></article>`).join('');
 ...
 $('countrySummary').innerHTML=`${label(data.workspace.country_label)} (${esc(data.workspace.country_code)}) · ${esc(data.workspace.default_currency)}. ${label(data.workspace.country_pack_stage)}.`;
...
$('language').value=language;$('language').addEventListener('change',event=>{language=['sr','en','bg'].includes(event.target.value)?event.target.value:'sr';localStorage.setItem(LANGUAGE_KEY,language);localizePortal();if(controlBriefData)renderControlBrief(controlBriefData);if(financialOverviewData)renderFinancialOverview(financialOverviewData);});$('logout').addEventListener('click',()=>{session=null;sessionStorage.removeItem('opsnestWorkspaceSession');$('loginView').classList.remove('hidden');$('appView').classList.add('hidden');$('logout').classList.add('hidden');$('password').value='';localizePortal();});localizePortal();if(session)load();
const portalObserver=new MutationObserver(()=>{portalObserver.disconnect();localizePortal();portalObserver.observe(document.body,{childList:true,subtree:true,characterData:true});});portalObserver.observe(document.body,{childList:true,subtree:true,characterData:true});
</script></body></html>""".replace("__OPSNEST_BG_TRANSLATIONS__", json.dumps(BG_TRANSLATIONS, ensure_ascii=False).replace("</", "<\\/"))
