...
function renderDocuments(data){const enabled=Boolean(data.storage.enabled),permissions=data.permissions||{},allowed=Array.isArray(permissions.visible_document_types)?permissions.visible_document_types:[],canUpload=Boolean(permissions.can_upload);$('documentForm').classList.toggle('hidden',!enabled||!canUpload);$('documentType').innerHTML=allowed.map(value=>`<option value="${esc(value)}">${esc(documentTypeLabel[value]||value)}</option>`).join('');if(!enabled){$('documentStatus').className='muted';$('documentStatus').textContent='Document storage is not enabled yet. No files can be uploaded until the private bucket is configured.';}else if(!canUpload){$('documentStatus').className='muted';$('documentStatus').textContent='Your role does not have access to the document archive.';}$('documentList').innerHTML=data.documents.map(document=>`<article class="queue-item"><div><b>${esc(document.original_filename)}</b><p>${esc(document.document_type)} · ${documentSize(document.byte_size)} · ${esc(document.uploaded_by_name)} · ${esc(document.created_at.replace('T',' '))}</p></div><div class="queue-actions"><button class="quiet small download-document" data-id="${esc(document.id)}" type="button">Download</button></div></article>`).join('')||'<p class="muted">No uploaded documents.</p>';localizePortal();document.querySelectorAll('.download-document').forEach(button=>button.addEventListener('click',()=>downloadDocument(button.dataset.id)));}
