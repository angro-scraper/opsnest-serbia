...
function tr(value){const source=String(value??'');return language==='sr'?(SR_TRANSLATIONS[source]||SR_DASHBOARD_TRANSLATIONS[source]||(source==='days'?'dana':source)):source;}
...
async function load(){try{showApp(await api('/v1/workspace/overview'));await Promise.all([loadWorkflow(),loadDocuments(),loadFinancialOverview(),loadTeam(),loadAudit()]);}catch(error){session=null;sessionStorage.removeItem('opsnestWorkspaceSession');$('loginView').classList.remove('hidden');$('appView').classList.add('hidden');$('logout').classList.add('hidden');$('loginError').textContent=error.message;}}
